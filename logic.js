import { addCondition, changeTurn, getCondition, getTurn, getWinner, updateScore } from "./data.js";
...
export const addLineToSquare = (line) => {
  ...
  if (line.className == "yline") {
    ...
    if (j != 9) addCondition(i / 2, Math.ceil(j / 2));
  } else if (line.className == "xline") {
    if (i != 1) addCondition(Math.floor(i / 2), j / 2);
    if (i != 9) addCondition(Math.ceil(i / 2), j / 2);
  }
};
